...
from utils.losses import gradient_loss, ncc_loss, crossentropy
from utils.utils import AverageMeter, to_categorical, dice
import numpy as np

class RSeg(object):
    def __init__(self, k=0, n_channels=1, n_classes=8, lr=1e-4, epoches=200, iters=200, batch_size=1, model_name='pcreg_5shot'):
        ...
        self.iters=iters
        self.lr = lr
        train_labeled_unlabeled_dir = 'data/heart_CT_5/train_labeled_unlabeled'
        train_unlabeled_unlabeled_dir = 'data/heart_CT_5/train_unlabeled_unlabeled'
        test_labeled_labeled_dir = 'data/heart_CT_5/test'
        ...
        # 初始化网络模型
        self.Reger = UNet_reg(n_channels=n_classes-1)
    ...
    def train_iterator_stage1(self, mi, fi, ml=None, fl=None):
        ...
        # train Reger
        w_m_to_f, w_label_m_to_f, flow = self.Reger(r_mi, r_fi, ml, fl)
    ...
    def train_iterator_stage2(self, mi, fi, ml=None, fl=None):
        for p in self.Seger.parameters():  # reset requires_grad
            p.requires_grad = False  # they are set to True below in Seger update
        for p in self.Reger.parameters():  # reset requires_grad             -
