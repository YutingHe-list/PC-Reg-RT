...
flag = 1
output_path = "./check/"
check_dir = "./check/TDReg"

ngf = 32
ndf = 64

train_dir = "LPB40/stage1"
...
class PCReg():
    def model_setup(self, model, is_training, z):
        ...
        with tf.variable_scope("Model1") as scope:
            self.output_A = unet(self.input_A, train=is_training, name="att_A")# 得到b的结构a的灰度值
        ...
        with tf.variable_scope("Model2") as scope:
            self.warp_AtoB, self.flow_AtoB, self.warp_AtoB_org, self.warp_gt_AtoB= build_generator_unet(self.feature_A, self.label_A_org,
                                                                                       self.input_A_org,
                                                                                       self.feature_B,
                                                                                       nf_enc,
                                                                                       nf_dec,
    ...
    def loss_calc(self):
        ...
        reg_loss_img1 = cc3D(self.warp_AtoB[:, :, :, :, 1], self.input_B * self.label_B1)
        reg_loss_img2 = cc3D(self.warp_AtoB[:, :, :, :, 2], self.input_B * self.label_B2)
        reg_loss_img3 = cc3D(self.warp_AtoB[:, :, :, :, 3], self.input_B * self.label_B3)
        reg_loss_img4 = cc3D(self.warp_AtoB[:, :, :, :, 4], self.input_B * self.label_B4)
        gradient = gradientLoss(self.flow_AtoB, 'l2')
        reg_loss_img = reg_loss_img1+reg_loss_img2+reg_loss_img3+reg_loss_img4+gradient
        reg_loss_img = tf.Print(reg_loss_img, [reg_loss_img], 'reg_loss_img')


        joint_loss1 = seg_loss_B+seg_loss_A
        joint_loss1 = tf.Print(joint_loss1, [joint_loss1], 'joint_loss1')

        joint_loss2 = reg_loss_img
        ...
        with tf.control_dependencies(extra_update_ops):
            self.joint_trainer1 = optimizer.minimize(joint_loss1, var_list=S_A_vars)
            self.joint_trainer2 = optimizer.minimize(joint_loss2, var_list=g_A_vars)
        for var in self.model_vars: print(var.name)
        self.joint_loss_sum1 = tf.summary.scalar("joint_loss1", joint_loss1)
        self.joint_loss_sum2 = tf.summary.scalar("joint_loss2", joint_loss2)
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            sess.run(init)
            if to_restore:
                # chkpt_fname = os.path.join(check_dir, 'model-199')
            ...
            # Training Loop
            for epoch in range(sess.run(self.global_step), 150):
                ...
                train_example_gen = example_gen_3D_brain(train_vol_names, bs=batch_size, data_aug=True)
                ...
                for ptr in range(0, iters):
                    ...
                    _, summary_str4 = sess.run([self.joint_trainer1, self.joint_loss_sum1],
                                                   feed_dict={self.label_B: data[2],
                                                              ...
                                                              self.lr: curr_lr})
                    _, summary_str5 = sess.run([self.joint_trainer2, self.joint_loss_sum2],
                                               feed_dict={self.label_B: data[2],
                                                          self.label_A: data[3],
                                                          self.input_A_org: self.mov_input,
                                                          self.input_A: self.mov_input,
